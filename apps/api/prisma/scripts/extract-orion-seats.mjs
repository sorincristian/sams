...
function isMatch(desc) {
  ...
  for (const s of SKIPPERS) if (d.includes(s)) return false;
  for (const m of MATCHERS) if (d.includes(m)) return true;
  return false;
}
        ...
        if (matches) {
           for (const potentialPart of matches) {
              ...
              if (existingSkus.has(part)) {
                 ...
                 if (cat === "CUSHION") newCushions.set(part, { desc: line.substring(0, 50), qty, file: fn });
                 if (cat === "BACK") newBacks.set(part, { desc: line.substring(0, 50), qty, file: fn });
              }
           }
        }
      }
