...
(async () => {
  ...
  try {
    ...
    // 2. Catalog fetch succeeds (Wait for API calls to settle)
    ...
    let initialRowCount = await page.locator('table tbody tr').count();

    // 3. Dropdown opens on focus
    await inputLocator.click();
    await page.waitForTimeout(500);
    let dropdownItems = page.locator('div[style*="max-height: 300px"] > div');
    const dbCount = await dropdownItems.count();
    if (dbCount > 1) {
       report.focusOpen = "PASS";
       // 4. Empty query shows all parts
       report.emptyShowAll = "PASS";
    }

    // 5 & 6. Filtering
    await inputLocator.fill('PN'); // part number mock
    await page.waitForTimeout(300);
    let filteredCount = await page.locator('div[style*="max-height: 300px"] > div').count();
    if (filteredCount > 0 && filteredCount <= dbCount) report.partNumFilter = "PASS";

    await inputLocator.fill('');
    await page.waitForTimeout(300);
    await inputLocator.fill('Seat'); // desc mock
    await page.waitForTimeout(300);
    let descCount = await page.locator('div[style*="max-height: 300px"] > div').count();
    ...
    report.defects.push(err.message);
  } finally {
